...
window.CampaignManager = (function() {
  ...
  function load() {
    ...
    if(el) el.innerHTML = '<tr><td colspan="14" style="text-align:center;padding:32px;color:var(--muted)">Loading campaigns...</td></tr>';
    if(typeof window.cmFetch === 'function') {
      window.cmFetch('/api/campaigns/all', function(err, d) {
        if(err) {
          console.error('CM load error:', err);
          var el = document.getElementById('cm-table-body');
          if(el) el.innerHTML = '<tr><td colspan="14" style="text-align:center;padding:32px;color:#dc2626">Error loading campaigns. <button onclick="CampaignManager.load()" style="border:none;background:var(--teal);color:var(--ink);padding:6px 12px;border-radius:6px;cursor:pointer;font-weight:600">Retry</button></td></tr>';
          return;
        }
        _data = d.campaigns || [];
        buildFilterOptions();
      ...
      xhr.open('GET', '/api/campaigns/all', true);
      xhr.setRequestHeader('Authorization', 'Bearer ' + _tok);
      ...
      xhr.onreadystatechange = function() {
        ...
        if(xhr.status === 200) {
          try {
            ...
            applyFilters();
            render();
          } catch(e) { console.error('CM parse error:', e); }
        }
      };
      xhr.send();
    }
  ...
  function render() {
    ...
    if(!tbody) return;
    if(!_filtered.length) {
      tbody.innerHTML = '<tr><td colspan="14" style="text-align:center;padding:48px;color:var(--muted)"><div style="font-size:28px;margin-bottom:8px">📭</div><div style="font-weight:600">No campaigns match your filters</div></td></tr>';
      return;
    }
    tbody.innerHTML = _filtered.map(function(c) {
      ...
      return '<tr style="border-bottom:1px solid var(--border);cursor:pointer;'+rowBg+'" onclick="CampaignManager.toggleRow(event,'+c.id+')">'
        ...
        + '<td style="padding:10px 12px;font-size:11px;color:var(--muted)">'+fmtDate(c.flight_end)+'</td>'
        + '<td style="padding:10px 12px"><div style="display:flex;gap:4px">'
          ...
          + (c.status==='active' ? '<button onclick="CampaignManager.pauseCampaign(event,'+c.id+')" style="border:none;background:#dbeafe;color:#1e40af;font-size:11px;font-weight:700;padding:4px 8px;border-radius:6px;cursor:pointer" title="Pause">⏸ Pause</button>' : '')
          + (c.status==='active'||c.status==='paused' ? '<button onclick="CampaignManager.stopCampaign(event,'+c.id+')" style="border:none;background:#fee2e2;color:#dc2626;font-size:11px;font-weight:700;padding:4px 8px;border-radius:6px;cursor:pointer" title="Stop">⏹ Stop</button>' : '')
        + '</div></td>'
        + '</tr>';
    }).join('');
  ...
  function setCampaignStatus(e, id, status) {
    e.stopPropagation();
    var camp = _data.find(function(c){ return c.id === id; });
    if(!camp) return;
    fetch('/api/campaign/'+id, {
      ...
      showToast('Campaign ' + (status==='paused'?'paused':'stopped'));
      load();
    }).catch(function() { showToast('Error updating campaign'); });
  }

  function pauseCampaign(e, id) { setCampaignStatus(e, id, 'paused'); }
  function stopCampaign(e, id) {
    if(!confirm('Stop this campaign? This will archive it.')) return;
    setCampaignStatus(e, id, 'archived');
  }
  ...
  return {
    ...
    pauseCampaign: pauseCampaign, stopCampaign: stopCampaign,
